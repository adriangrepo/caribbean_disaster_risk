...
def get_angle(poly):
    ...
    if len(poly)>0:
        if isinstance(poly.iloc[0], MultiPolygon):
            #ex=poly.convex_hull.exterior
            #print(type(ex))
            #clr=list(shape(ex).coords)
            #clr = list(ex.coords)
            #c=list(lr.coords)
...
def mask_raster(df, tif_filename, country, region, save_path, buffer=2, debug=False):
    angles={}
    print(f'mask_raster path_out: {data_dir}/{country}_{region}/cropped/{save_path}')
    print(f'mask_raster rot_out: {data_dir}/{country}_{region}/cropped/{save_path}/rotated/')
    with rasterio.open(tif_filename) as src:
        for index, row in df.iterrows():
            ...
            #print(f'mask_raster rotated.shape: {rotated.mode}')
...
def mask_images(df, df_test, data_tif, test_exists, country, region, buffer, debug=False):
    mask_raster(df, data_tif, country, region, save_path='train', buffer=buffer, debug=debug)

    if test_exists:
        mask_raster(df_test, data_tif, country, region, save_path='test', buffer=buffer, debug=debug)
