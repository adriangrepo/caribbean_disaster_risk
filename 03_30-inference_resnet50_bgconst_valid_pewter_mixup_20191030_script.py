...
def setup_basic_dataset(df_train, tfms):
...
def confusion_loss(learn, preds, pred_classes, tl_val, tl_idx, losses, arch_name, combo_name, run_num, model_file):
...
def create_preds(run, model_file, arch_name, learn, combo_name):
    mname=f'{model_file}'
    learn.load(f'{mname}')
    interp = ClassificationInterpretation.from_learner(learn)
    preds, pred_classes, tl_val, tl_idx, losses=get_top_losses(interp, k=len(interp.losses))
    confusion_loss(learn, preds, pred_classes, tl_val, tl_idx, losses, arch_name, combo_name, run, model_file)
...
def model_inference(data, runs, model_file, arch, arch_name, combo_name):
    learn=create_model(data, arch)
    create_preds(runs, model_file, arch_name, learn, combo_name)
...
def all_runner():
    # stage-2-rn50-03_30-bg_const-20191201-fb9c67aa-all
    background = 'constant'
    ...
    for i in range(num_splits):
        run_code = 'r' + str(i)
        model_file=f'{nb_num}-{uid}_{special_code}-{final_stage}-{run_code}-{date}'
        run_all(split_list[i], i, model_file, arch, arch_name, combo_name)
...
def runner():
    ...
    for run in range(runs):
        ...
        model_inference(data, runs, model_file, arch, arch_name, combo_name)

        df, non_dup_df=load_preds(model_file,combo_name,plot=False)
